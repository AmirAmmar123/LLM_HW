...
def train_and_evaluate(X, y, task_name, feature_name):
    ...
    lr_best, best_params, best_score = tune_logistic_regression(X, y)

    y_pred_lr = cross_val_predict(lr_best, X, y, cv=5, n_jobs=1)

    logger.info("\n--- Logistic Regression (Optimized) ---")
    logger.info(f"Best params: {best_params}, Best F1={best_score:.4f}")
