...
class BinaryClassificationTask:
    ...
    def create_features(self):
        ...
        logger.info(f"Testing MINDEF: {self.MINDEF[0]}, MAXDEF: {self.MAXDEF[0]}, Ngram: {self.UNIGRAMS}")
        text_data = [r['sentence_text'] for r in self.records]
        self.vectorizer_bow = CountVectorizer(min_df=self.MINDEF[0], max_df=self.MAXDEF[0], ngram_range=self.UNIGRAMS)
        self.X_bow = self.vectorizer_bow.fit_transform(text_data)
        logger.info(f"BOW Vector shape: {self.X_bow.shape}")

        self.vectorizer_tfidf = TfidfVectorizer(min_df=self.MINDEF[0], max_df=self.MAXDEF[0], ngram_range=self.UNIGRAMS)
