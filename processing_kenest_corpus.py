...
def process_file(args):
    ...
    if result:
        ...
        protocol = Protocol(knesset_number, protocol_type, filename, base_dir)
...
class Protocol:
    def __init__(self, knesset_number: int, protocol_type: str, filename: str, base_dir):
...
#     tasks = [(f, args.input_dir) for f in files]
...
if __name__ == "__main__":
    ...
    for idx, filename in enumerate(files, 1):
        result = process_file((filename, args.input_dir))
        if result:
